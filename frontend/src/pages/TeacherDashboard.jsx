...
const TeacherDashboard = () => {
  ...
  const shortcuts = [
    { to: "/teacher/classes", title: "Classes & Students", description: "Assign CRs and manage rosters", icon: Users },
    ...
    <main className="mx-auto max-w-6xl px-6 py-8">
      ...
      {loading ? <div className="flex justify-center py-20"><Spinner /></div> : <>
        ...
        <div className="mt-7 grid grid-cols-1 gap-6 lg:grid-cols-2">
          <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-100"><div className="flex items-center justify-between gap-3"><h3 className="font-semibold text-slate-800">Pending today</h3><Link to="/teacher/attendance" className="text-sm font-medium text-brand-600">View attendance</Link></div>{dashboard?.pendingClasses?.length ? <div className="mt-4 space-y-3">{dashboard.pendingClasses.map((item) => <div key={item.id} className="flex items-center justify-between rounded-xl bg-amber-50 px-4 py-3 text-sm"><div><p className="font-medium text-slate-700">{item.className} · Section {item.section}</p><p className="text-xs text-slate-500">{item.year} · {item.crName}</p></div><span className="text-xs font-medium text-amber-700">Not submitted</span></div>)}</div> : <p className="mt-4 rounded-xl bg-emerald-50 p-4 text-sm text-emerald-700">All assigned classes have submitted attendance today.</p>}</section>
